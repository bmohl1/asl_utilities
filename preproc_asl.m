...
for n = 1:length(pth_subjdirs)
    ...
    for t =1:length(tasks)
        ...
        cd (strcat(proj_dir, filesep, subj));
        ...
        for z = 1:length(rawDirs)
            ...
            raw_dir = strcat(cwd,filesep,subj,filesep,rawDirs(z).name);
            cd(input_dir)
            ...
            val=cellfun(@(x) numel(x),{rawFileName}); %compare the length of all the nii's
            ...
            if isempty(check_if_processed_name) || eq(ignore_preproc,1)
                ...
                matlabbatch{jj}.spm.util.defs.out{1}.pull.fnames = {strcat(task_dir,filesep,img2norm)};
                ...
                matlabbatch{jj}.spm.spatial.smooth.data(1) = cfg_dep('Deformations: Warped Images', substruct('.','val', '{}',{3}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','warped'));
            ...
            if eq (save_qc,1) 
                ...
                if eq(baseimg.dim, [121 145, 121])
                    dims = [1.5 1.5 1.5 ];
                elseif eq(baseimg.dim, [91 109 91])
                    dims = [2 2 2];
                else
                    dims = [3 3 3];
                end
                ...
                masks = {'q1','q2','q3','q4','q5','q6','bottom','top'};
                for m = 1:length(masks)
                    mask_loc = dir(strcat(qc_mask_dir,filesep,masks{m},'*'));
                    ...
                    if ~eq(baseimg.dim, maskimg.dim)  % The resizing is essential to make sure the matrix sizes match for the "add_data" line
                        resize_img(maskimg.fname, dims, [nan nan nan; nan nan nan]);
                        maskimg = spm_vol(strcat(qc_mask_dir,filesep,'r',mask_loc.name));
                    end
                    maski_data = spm_read_vols(maskimg);
                    add_data = (basei_data.*maski_data);
                    fprintf(fileID,'%s %6f %3f %3f\n',masks{m},mean2(add_data(add_data~=0)),std2(add_data(add_data~=0)),max(add_data(:)));
                    if mean2(add_data(add_data~=0)) > 250 %25ml/100g/min for mixed tissue types
                ...
                for s = 1:length(segs)
                    seg_loc = dir(strcat(task_dir,filesep,segs{s},'*'));
                    if ~isempty (seg_loc);
                        segimg = spm_vol(seg_loc(1).name);
                        segi_data=spm_read_vols(segimg);
                        fprintf(fileID,'%s %6f %3f %3f \n',segs{s},mean2(segi_data(segi_data~=0)),std2(segi_data(segi_data~=0)), max(segi_data(:)));
                        if mean2(segi_data(segi_data~=0)) > 800 %80ml/100g/min
        ...
        %    log_file = [cwd filesep dt '_aslCortSummary.txt']
